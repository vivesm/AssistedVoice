...
INTENT_PATTERNS = {
    ...
    'docker': [
        r'\b(docker|container|image|ps|logs|inspect)\b',
        r'\bwhat containers are running\b',
    ],
    'desktop': [
        r'\b(run|execute|shell|terminal|process|kill)\b',
        r'\b(show|list) processes\b',
    ],
    'memory': [
        r'\b(remember|memory|knowledge graph|entity|relation)\b',
...
class ChatService:
    ...
    def _detect_intent(self, prompt: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        if prompt_lower.startswith(('docker ', 'container ')):
             for prefix in ['docker ', 'container ']:
                if prompt_lower.startswith(prefix):
                    return 'docker', prompt[len(prefix):].strip()

        if prompt_lower.startswith(('run ', 'exec ', 'shell ')):
             for prefix in ['run ', 'exec ', 'shell ']:
                if prompt_lower.startswith(prefix):
                    return 'desktop', prompt[len(prefix):].strip()

        if prompt_lower.startswith(('remember ', 'recall ')):
             for prefix in ['remember ', 'recall ']:
                if prompt_lower.startswith(prefix):
                    return 'memory', prompt[len(prefix):].strip()

        if prompt_lower.startswith(('think ', 'analyze ')):
             for prefix in ['think ', 'analyze ']:
                if prompt_lower.startswith(prefix):
                    return 'thinking', prompt[len(prefix):].strip()
...
Based on this page content, provide a helpful answer."""
    ...
    def _augment_with_docker(self, prompt: str, query: str) -> str:
        """Augment prompt with Docker container information"""
        docker = self.mcp_services.get('docker')
        if not docker or not docker.is_available():
            return prompt
        
        logger.info(f"🐳 Checking Docker for: {query}")
        # Simple logic: if query contains 'logs', show logs, else list containers
        if 'logs' in query.lower():
            words = query.split()
            container = words[-1] if words else None
            output = docker.get_container_logs(container) if container else docker.list_containers()
        else:
            output = docker.list_containers()
...
Provide an update based on this Docker information."""

    def _augment_with_desktop(self, prompt: str, query: str) -> str:
        """Augment prompt with system command execution"""
        desktop = self.mcp_services.get('desktop')
        if not desktop or not desktop.is_available():
            return prompt
        
        logger.info(f"🖥️ Executing desktop command: {query}")
        if 'process' in query.lower():
            output = desktop.list_processes()
        else:
            output = desktop.execute_command(query)
...
Provide an update based on this command output."""

    def _augment_with_memory(self, prompt: str, query: str) -> str:
        """Augment prompt with knowledge graph memory"""
        memory = self.mcp_services.get('memory')
...
Based on what you remember, provide a helpful answer."""

    def _augment_with_thinking(self, prompt: str, query: str) -> str:
        """Augment prompt with sequential thinking"""
        thinking = self.mcp_services.get('thinking')
        if not thinking or not thinking.is_available():
            return prompt
        
        logger.info(f"💭 Thinking deeply about: {query}")
        process = thinking.think(query)
            ...
            if self._tools_enabled and not images:
                ...
                if tool_type and query:
                    if tool_type == 'search':
                        ...
                        prompt = self._augment_with_docker(prompt, query)
                    elif tool_type == 'desktop':
                        prompt = self._augment_with_desktop(prompt, query)
